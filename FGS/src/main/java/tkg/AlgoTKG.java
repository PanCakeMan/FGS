...
//	private List<FrequentSubgraph> frequentSubgraphs;
	...
	public void runAlgorithm(String inPath, String outPath, int k, boolean outputSingleVertices,
		...
		// read graphs
		List<MGraph> MMGraphDB = readGraphs(inPath);

		// Calculate the minimum support as a number of graphs
		minSup = 1;

		// mining
		gSpan(MMGraphDB, outputSingleVertices);
...
//				System.out.println(minSup);
	...
	private List<MGraph> readGraphs(String path) throws IOException {
		...
		List<MGraph> MMGraphDatabase = new ArrayList<>();
...
//                    System.out.println(v1 + " " + v2 + " " + vMap.get(v1).id + " " + vMap.get(v2).id);
			...
			MMGraphDatabase.add(new MGraph(gId, vMap));
			if (line != null) {
				hasNextGraph = true;
			}
		}
		...
		if (DEBUG_MODE) {
			System.out.println("read successfully, totally " + MMGraphDatabase.size() + " graphs");
		}
		graphCount = MMGraphDatabase.size();
		return MMGraphDatabase;
	}
	...
	private Map<ExtendedEdge, Set<Integer>> rightMostPathExtensions(DFSCode c, List<MGraph> MMGraphDatabase,
...
//        	int remaininggraphCount = graphIds.size();
			for (Integer graphId : graphIds) {
				MGraph g = MMGraphDatabase.get(graphId);
...
//            		System.out.println("BREAK2");
	 ...
	 * 
	 * @param MMGraphDB                a graph database
	...
	private void gSpan(List<MGraph> MMGraphDB, boolean outputFrequentVertices) throws IOException, ClassNotFoundException {
		...
		for (int i = 0; i < MMGraphDB.size(); i++) {
			MGraph g = MMGraphDB.get(i);
		...
		if (frequentVertexLabels.size() != 0) {
			...
			if (DYNAMIC_SEARCH) {
				...
				gSpanDynamicDFS(new DFSCode(), MMGraphDB, graphIds);
				
				if(THREADED_DYNAMIC_SEARCH) {
					for(int i=0; i< THREAD_COUNT; i++) {
						Thread thread = new DFSThread(MMGraphDB);
						thread.start();
					}
					...
					while (candidates.size() > 0) {
						...
						gSpanDynamicDFS(candidate.dfsCode, MMGraphDB, candidate.setOfGraphsIDs);
				...
				gSpanDFS(new DFSCode(), MMGraphDB, graphIds);
			}
		}
	}
	
	class DFSThread extends Thread{
		List<MGraph> MMGraphDB;
		
		public DFSThread(List<MGraph> MMGraphDB) {
			this.MMGraphDB = MMGraphDB;
		}
	 ...
	 */
	private void removeInfrequentVertexPairs(List<MGraph> MMGraphDB) {
	 ...
	 * @param MMGraphDB  the graph database
	 * @param graphIds the ids of graph where the graph "c" appears
	 * @throws IOException            exception if error writing/reading to file
	 * @throws ClassNotFoundException if error casting a class
	 */
	private void gSpanDFS(DFSCode c, List<MGraph> MMGraphDB, Set<Integer> graphIds)
			throws IOException, ClassNotFoundException {
		...
		Map<ExtendedEdge, Set<Integer>> extensions = rightMostPathExtensions(c, MMGraphDB, graphIds);
		...
		for (Entry<ExtendedEdge, Set<Integer>> entry : extensions.entrySet()) {
			...
			if (sup >= minSup) {
				...
				if (isCanonical(newC)) {
					...
					gSpanDFS(newC, MMGraphDB, newGraphIDs);
				}
			}
		}
	 ...
	 * @throws ClassNotFoundException if error casting a class
	 */
	private void gSpanDynamicDFS(DFSCode c, List<MGraph> MMGraphDB, Set<Integer> graphIds)
			throws IOException, ClassNotFoundException {
		// If we have reached the maximum size, we do not need to extend this graph
		if (c.size() == maxNumberOfEdges - 1) {
			return;
	 ...
	 */
	private void findAllOnlyOneVertex(List<MGraph> MMGraphDB, boolean outputFrequentVertices) {
		...
		for (Entry<Integer, Set<Integer>> entry : labelM.entrySet()) {
			...
			if (sup >= minSup) {
				...
				for (Integer graphid : tempSupG) {
					MGraph g = MMGraphDB.get(graphid);
