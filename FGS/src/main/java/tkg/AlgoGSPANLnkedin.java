...
public class AlgoGSPANLnkedin {
	...
	public void runAlgorithm(String inPath, String outPath, double minSupport, boolean outputSingleVertices,
		...
		// read graphs
		List<MGraph> MGraphDB = readGraphs(inPath);

		// Calculate the minimum support as a number of graphs
		minSup = (int) Math.ceil(minFrequency * MGraphDB.size());

		// mining
		gSpan(MGraphDB, outputSingleVertices);
	...
	public ArrayList<Graph<DataNode, RelationshipEdge>> runAlgorithm(ArrayList<Graph<DataNode, RelationshipEdge>> res, String outPath, double minSupport, boolean outputSingleVertices,
							  boolean outputDotFile, int maxNumberOfEdges, boolean outputGraphIds,
							  HashMap<Integer, String> nodeLabelMap, HashMap<Integer, String> edgeLabelMap)
			throws IOException, ClassNotFoundException {
		...
		// read graphs
		List<MGraph> MGraphDB = readGraphs(res,edgeLabelMap);

//		for (int i = 0; i < MGraphDB.size(); i++) {
//
//			for (int key : MGraphDB.get(i).vMap.keySet()) {
//
//				if (nodeLabelMap.containsKey(MGraphDB.get(i).vMap.get(key).getLabel())) {
//					System.out.println(nodeLabelMap.get(MGraphDB.get(i).vMap.get(key).getLabel()));
//				}
//
//			}
//			System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!");
//		}
		...
		ArrayList<Graph<DataNode, RelationshipEdge>> candidatePatterns =
				getCandidatePatterns(frequentSubgraphs,nodeLabelMap,edgeLabelMap);

//		for (Graph<DataNode, RelationshipEdge> pattern: candidatePatterns) {
...
//				System.out.println(pattern.getEdgeSource(edge).getNodeName() + "--->" + pattern.getEdgeTarget(edge).getNodeName() + " " + edge.getLabel());
//			}
//			System.out.println("+++++++++++++++++");
//		}
	...
	public ArrayList<Graph<DataNode, RelationshipEdge>> getCandidatePatterns (List<FrequentSubgraph> patterns,
																			  HashMap<Integer,String> nodeLabelMap,
																			  HashMap<Integer,String> edgeLabelMap) {

		ArrayList<Graph<DataNode, RelationshipEdge>> candidatePatterns = new ArrayList<>();
		int i = 0;
		for (FrequentSubgraph subgraph : patterns) {
			...
			if (dfsCode.size() == 1) {
				...
				if (ee.getEdgeLabel() == -1) {
					...
					String type0 = nodeLabelMap.get(ee.getvLabel1());
					if (!type0.equals("User")) {
						continue;
					}
					String type1 = nodeLabelMap.get(ee.getvLabel2());
					node0.types.add(type0);
					node1.types.add(type1);
					String edgeLabel = edgeLabelMap.get(ee.getEdgeLabel());
					edgeLabel = edgeLabel.replace("<","");
					edgeLabel = edgeLabel.replace(">","");
					pattern.addVertex(node0);
					pattern.addVertex(node1);
					pattern.addEdge(node0,node1,new RelationshipEdge(edgeLabel));
					candidatePatterns.add(pattern);
				}
			} else {
				HashMap<Integer,DataNode> nodeMap = new HashMap<>();
				List<Integer> vLabels = dfsCode.getAllVLabels();
				if (!nodeLabelMap.get(vLabels.get(0)).equals("User")) {
					continue;
				}
				for (int j = 0; j < vLabels.size(); j++) {

					DataNode node = new DataNode(String.valueOf(j));
					node.types.add(nodeLabelMap.get(vLabels.get(j)));
					nodeMap.put(j,node);
					pattern.addVertex(node);
				}
				for (ExtendedEdge ee : dfsCode.getEeL()) {
					DataNode startV = nodeMap.get(ee.getV1());
					DataNode endV = nodeMap.get(ee.getV2());
					String edgeLabel = edgeLabelMap.get(ee.edgeLabel);
					edgeLabel = edgeLabel.replace("<","");
					edgeLabel = edgeLabel.replace(">","");
//					System.out.println(edgeLabel);
					pattern.addEdge(startV,endV,new RelationshipEdge(edgeLabel));
				}
			}
        ...
        System.out.println(candidatePatterns.size()+ "+++++++++++++++++++++++++++");
		return candidatePatterns;
	}
	 ...
	 */
	private List<MGraph> readGraphs(ArrayList<Graph<DataNode, RelationshipEdge>> res,HashMap<Integer,String> nodeLabelMap) {

		if (DEBUG_MODE) {
			System.out.println("start reading graphs...");
		}

		List<MGraph> MGraphDatabase = new ArrayList<>();
		// For each graph of the graph database
		for (int i = 0; i < res.size(); i++) {
			Graph<DataNode, RelationshipEdge> curGraph = res.get(i);
			System.out.println("Transforming graph" + i);
			int gId = i;
			Map<Integer, Vertex> vMap = new HashMap<>();


			for (DataNode node : curGraph.vertexSet()) {
				int vId = node.getNodeName().hashCode();
				int vLabel = node.types.toArray()[0].hashCode();

				vMap.put(vId, new Vertex(vId, vLabel));
			}

			for (RelationshipEdge edge : curGraph.edgeSet()) {
				int v1 = curGraph.getEdgeSource(edge).getNodeName().hashCode();
				int v2 = curGraph.getEdgeTarget(edge).getNodeName().hashCode();
				int label = edge.getLabel().hashCode();
				Edge e = new Edge(v1, v2, label);
				vMap.get(v1).addEdge(e);
				vMap.get(v2).addEdge(e);
			}
			MGraphDatabase.add(new MGraph(gId, vMap));
//			for (Vertex v :vMap.values()) {
//				System.out.println(edgeLabelMap.get(v.getLabel()));
//			}
		...
		if (DEBUG_MODE) {
			System.out.println("read successfully, totally " + MGraphDatabase.size() + " graphs");
		}
		graphCount = MGraphDatabase.size();
		System.out.println("Graph List Size" + graphCount);
		return MGraphDatabase;
	}


	private List<MGraph> readGraphs(String path) throws IOException {
		if (DEBUG_MODE) {
			System.out.println("start reading graphs...");
		}
...
//                    System.out.println(v1 + " " + v2 + " " + vMap.get(v1).id + " " + vMap.get(v2).id);
	 ...
	 */
	private List<Map<Integer, Integer>> subgraphIsomorphisms(DFSCode c, MGraph g) {
	...
	private Map<ExtendedEdge, Set<Integer>> rightMostPathExtensionsFromSingle(DFSCode c, MGraph g) {
		int gid = g.getId();
	...
	private Map<ExtendedEdge, Set<Integer>> rightMostPathExtensions(DFSCode c, List<MGraph> MGraphDatabase,
...
//        	int remaininggraphCount = graphIds.size();
			for (Integer graphId : graphIds) {
				MGraph g = MGraphDatabase.get(graphId);
...
//            		System.out.println("BREAK2");
	 ...
	 * @param MGraphDB                a graph database
	 ...
	 */
	private void gSpan(List<MGraph> MGraphDB, boolean outputFrequentVertices) throws IOException, ClassNotFoundException {

		// If the user wants single vertex graph, we will output them
		if (outputFrequentVertices || ELIMINATE_INFREQUENT_VERTICES) {
			findAllOnlyOneVertex(MGraphDB, outputFrequentVertices);
		}


		for (MGraph g : MGraphDB) {
			g.precalculateVertexList();
		}

		if (ELIMINATE_INFREQUENT_VERTEX_PAIRS || ELIMINATE_INFREQUENT_EDGE_LABELS) {
			removeInfrequentVertexPairs(MGraphDB);
		}
		...
		if (frequentVertexLabels.size() != 0) {
			...
			gSpanDFS(new DFSCode(), MGraphDB, graphIds);
		}
	}
	 ...
	 */
	private void removeInfrequentVertexPairs(List<MGraph> MGraphDB) {
	 ...
	 * @param MGraphDB  the graph database
	 * @param graphIds the ids of graph where the graph "c" appears
	 * @throws IOException            exception if error writing/reading to file
	 * @throws ClassNotFoundException if error casting a class
	 */
	private void gSpanDFS(DFSCode c, List<MGraph> MGraphDB, Set<Integer> graphIds)
			throws IOException, ClassNotFoundException {
		...
		Map<ExtendedEdge, Set<Integer>> extensions = rightMostPathExtensions(c, MGraphDB, graphIds);
			...
			for (Entry<ExtendedEdge, Set<Integer>> entry : extensions.entrySet()) {
				...
				if (sup >= minSup) {
					...
					if (isCanonical(newC)) {
						...
						gSpanDFS(newC, MGraphDB, newGraphIDs);
					}
				}
			}
		}
	...
	private boolean isCanonical(DFSCode c) {
		...
		for (int i = 0; i < c.size(); i++) {
			Map<ExtendedEdge, Set<Integer>> extensions = rightMostPathExtensionsFromSingle(canC, new MGraph(c));
	 ...
	 */
	private void findAllOnlyOneVertex(List<MGraph> MGraphDB, boolean outputFrequentVertices) {
