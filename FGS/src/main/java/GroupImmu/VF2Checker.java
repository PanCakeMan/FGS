...
public class VF2Checker {
    ...
    public VF2Checker(CovidDataGraph linkedinGraph) {
...
//
    ...
    public void runMaxSumGen()
        ...
        for (DataNode node : dataGraph.vertexSet()) {
            ...
            if (((String) node.attributes.get("age").toArray()[0]).length() > 2) {
                continue;
            }

            if (Double.parseDouble((String) node.attributes.get("age").toArray()[0]) < 40) {
                oldGraph.addVertex(node);
            }
        }
        ...
        for (RelationshipEdge edge : dataGraph.edgeSet()) {
            ...
            System.out.println(s.attributes.get("age").toArray()[0]);
            System.out.println(t.attributes.get("age").toArray()[0] + "---->");


            if (((String) s.attributes.get("age").toArray()[0]).length() > 2) {
                continue;
            }
            if (((String) t.attributes.get("age").toArray()[0]).length() > 2) {
                continue;
            }

            if (Double.parseDouble((String) s.attributes.get("age").toArray()[0]) < 40 && Double.parseDouble((String) t.attributes.get("age").toArray()[0]) < 40) {
                 oldGraph.addEdge(s,t, new RelationshipEdge("contact"));
            }

        }

        System.out.println(oldGraph.vertexSet().size()+ "-----");
        System.out.println(oldGraph.edgeSet().size() + "----");

        ArrayList<Graph<DataNode, RelationshipEdge>> allGraphList = new ArrayList<>();
        allGraphList.add(dataGraph);
        OutputGraphGrami(allGraphList, "old.txt");
