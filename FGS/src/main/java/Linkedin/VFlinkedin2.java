...
public class VFlinkedin2 {
    ...
    public VFlinkedin2(LinkedinGraph dataGraph) {
...
//
                if (!canSkip) {
                    ...
                    ArrayList<Graph<DataNode, RelationshipEdge>> curMiningCandidates = algoGSPAN.runAlgorithm(graphAdd, "streamOut1.txt", 1,
...
//        OutputGraphGrami(targetGraphList, solution);
//        for (DataNode node : solution) {
    ...
    private boolean isExtenable(ArrayList<String> group, ArrayList<Rangepair> cc, HashSet<DataNode> temp, int m) {
        ...
        for (int i = 0; i < group.size(); i++) {
            ...
            for (DataNode node : temp) {
                if (!node.attributes.containsKey("Gender")||!node.attributes.containsKey("")) {
                    continue;
                }
                if (node.attributes.get("Gender").toArray()[0].equals(group.get(i))) {
                    count++;
...
//            System.out.println("add");
    ...
    public static void main(String args[]) throws IOException, ClassNotFoundException {
        ...
        VFlinkedin2 checker = new VFlinkedin2(graph);


        ArrayList<String> group = new ArrayList<>();
        group.add("male:Financial Services");
        group.add("female:Information Technology and Services");
        group.add("female:Financial Services");
        group.add("male:Information Technology and Services");

        ArrayList<Rangepair> cc = new ArrayList<>();
        cc.add(new Rangepair(40, 60));
        cc.add(new Rangepair(40, 60));

        long startTime = System.nanoTime();
        checker.runMaxSumGen(group, cc, 2, 100);
